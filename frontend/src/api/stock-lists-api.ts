...
export default class StockListsApi {
    ...
    static async getStockListStocks(username: string, listName: string) {
        const response = await HttpClient.get(`/stock-lists/stocks?username=${username}&listName=${listName}`);
        const data = await response.json();
    ...
    static async createStockList(token: string, listName: string, isPublic: boolean) {
    ...
    static async addStockToList(token: string, listName: string, symbol: string, amount: number) {
        const response = await HttpClient.post(
    ...
    static async removeStockFromList(token: string, listName: string, symbol: string) {
        const response = await HttpClient.post(
